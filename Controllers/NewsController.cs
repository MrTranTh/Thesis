...
namespace Thesis.Controllers
{
    public class NewsController : Controller
    {
        ...
        public NewsController(AppDbContext dbContext, IWebHostEnvironment env, SendMailService sendMailService)
		{
			db = dbContext;
            _env = env;
		...
		public IActionResult hienThiNew(int id)
		{
            TempData["Id"] = id; 
            var item = db.News.Where(x => x.Id == id).FirstOrDefault();
			ViewBag.noiDungGanDay = db.News.OrderByDescending(x=> x.CreatedDate).Take(3).ToList();
			return View(item);
		}
		...
		public IActionResult NhatXetKhachHang(string hoTen, string email, string diaChi, string loiNhan) 
		{
            ...
            return RedirectToAction("hienThiNew", new { id = id });
        }
	}
}
